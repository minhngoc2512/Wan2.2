...
def process_i2v(image_input, prompt, size, steps, seed, duration_sec, fps,
    ...
    try:
        # ── Clear VRAM cache before subprocess ──
        try:
            import gc
            torch.cuda.empty_cache()
            gc.collect()
        ...
        # ── Re-stamp FPS + re-encode browser-compatible ──
        final = os.path.join(tempfile.gettempdir(), "wan22_i2v_output.mp4")
        restamped = output_path.replace(".mp4", f"_{out_fps}fps.mp4")
        r = subprocess.run([
            "ffmpeg", "-y", "-i", output_path,
            "-vf", f"fps={out_fps}",
            "-c:v", "libx264", "-preset", "fast", "-crf", "18",
            "-pix_fmt", "yuv420p", "-movflags", "+faststart", restamped,
        ], capture_output=True, timeout=600)
        src = restamped if r.returncode == 0 and os.path.exists(restamped) else output_path
