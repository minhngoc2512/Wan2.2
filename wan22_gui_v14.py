...
def build_ui(ckpt_dir=DEFAULT_CKPT_DIR):
    with gr.Blocks(title="Wan2.2 Character Studio", js=AUTO_SCROLL_JS) as demo:
        ...
        # ── Server Status Banner (shown to all clients when busy) ──
        busy_banner = gr.HTML(
            value="",
            visible=False,
            elem_id="busy-banner",
        ...
        # Poll server status every 3 seconds for all connected clients
        ...
        # Poll server status every 3s — updates ALL connected clients simultaneously
        status_timer.tick(
