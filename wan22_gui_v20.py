...
def process_flf2v(first_image, last_image, prompt, size, steps, seed,
    ...
    try:
        ...
        # ── Load VAE (shared between both models) ──
        logs.append("  🔄  Loading VAE...")
        yield None, emit(logs)
        vae = AutoencoderKLWan.from_pretrained(
            ckpt_dir_high, subfolder="vae", torch_dtype=torch.float32
        )

        # ── Load High-Noise model (first frame) ──
        logs.append(f"  🔄  Loading High-Noise model ({ckpt_dir_high})...")
        yield None, emit(logs)
        pipe_high = WanImageToVideoPipeline.from_pretrained(
            ckpt_dir_high, vae=vae, torch_dtype=dtype
        )
        if cpu_offload:
            pipe_high.enable_model_cpu_offload()
        else:
            pipe_high.to("cuda")

        # ── Load Low-Noise model (last frame) ──
        logs.append(f"  🔄  Loading Low-Noise model ({ckpt_dir_low})...")
        yield None, emit(logs)
        pipe_low = WanImageToVideoPipeline.from_pretrained(
            ckpt_dir_low, vae=vae, torch_dtype=dtype
        )
        if cpu_offload:
...
def process_i2v(image_input, prompt, size, steps, seed, duration_sec,
    ...
    # frame_num from duration
    fps_map = {"1280x720": 16, "960x540": 16, "832x480": 16, "640x480": 16}
    out_fps = 16
    frame_num = max(1, round(duration_sec * out_fps))
    if (frame_num - 1) % 4 != 0:
        frame_num = ((frame_num - 1) // 4 + 1) * 4 + 1
    ...
    try:
        cmd = build_i2v_cmd(
            image_input, prompt, output_path, size, steps, seed,
        ...
        if proc.returncode != 0:
            logs += ["", f"❌  I2V FAILED (exit {proc.returncode})"]
            yield None, emit(logs)
            return
...
def process_t2v(prompt, size, steps, seed, duration_sec,
    ...
    try:
        ...
        if proc.returncode != 0:
            logs += ["", f"❌  T2V FAILED (exit {proc.returncode})"]
            yield None, emit(logs)
            return
          ...
          with gr.TabItem("🖼️  Image to Video"):
            ...
            i2v_run_btn.click(
                ...
                inputs=[i2v_image, i2v_prompt, i2v_size, i2v_steps, i2v_seed,
                        i2v_duration, i2v_ckpt, i2v_gpus, i2v_cpu_offload,
