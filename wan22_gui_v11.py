...
def process_video(video_input, character_input, mode, resolution, fps,
    ...
    try:
        ...
        # frame_num = ceil(video_duration_sec * output_fps)
        # Must be odd (Wan2.2 requires 4k+1 frames: 1,5,9,...,77,...)
        _duration = get_video_duration(video_input)
        if _duration > 0:
            _raw_frames = _duration * fps          # fps = output fps chosen by user
            _frame_num  = max(1, round(_raw_frames))
            # Wan2.2 needs frame_num = 4k+1 → round up to nearest
            if (_frame_num - 1) % 4 != 0:
                _frame_num = ((_frame_num - 1) // 4 + 1) * 4 + 1
        else:
            _frame_num = 77   # fallback default
        logs.append(f"  📐  Video duration: {_duration:.2f}s  →  frame_num: {_frame_num} @ {fps}fps")
        yield None, emit(logs)
